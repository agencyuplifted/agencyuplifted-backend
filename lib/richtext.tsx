...
// Sehr einfaches Markup: **text** wird fett dargestellt, ==text== als Akzent
// (Marken-Rot + fett, siehe --color-akzent-rot in globals.css -- so stellt es
// auch das Onepage-Frontend dar). Kein volles Markdown, bewusst minimal
// gehalten für Options-Beschreibungen und Features.
export function renderFett(text: string | null | undefined): React.ReactNode {
  ...
  return teile.map((teil, i) => {
    ...
    if (teil.startsWith("==") && teil.endsWith("==") && teil.length > 4) {
      return (
        <strong key={i} style={{ color: "var(--color-akzent-rot)" }}>
          {teil.slice(2, -2)}
        </strong>
      );
    }
