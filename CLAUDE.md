...
- `lib/funnel.ts` — trigger-based lifecycle email funnels (`buchung_erstellt`, `vor_seminarstart`, `nach_seminarende`, `lead_erstellt`, `warteliste_eingetragen`), placeholder rendering (`{{vorname}}` etc. via `renderPlatzhalter`), and the due-mail check invoked by the cron route. Pro Mail zusätzlich: Tag-Bedingungen (`tag_bedingung_mit/ohne`), Tag nach Versand (`tag_nach_versand`, nur Teilnehmer, quelle `automatisch`) und Anschluss-/Journey-Bedingungen (`nur_seminartyp_id`, `ausschluss_seminartyp_id` = schon besucht oder gebucht, optional `mindestabstand_tage` — 0 = kein Capping; innerhalb der Frist wird verschoben, nicht verworfen, weil der Stichtag im Fenster bleibt).
...
- `lib/mail-html.ts` / `lib/mail-bausteine.ts` / `lib/abmelden.ts` — gemeinsame Mail-Bausteine (Tabelle `mail_bausteine`, eine Zeile: Signatur, Firmenangaben, Impressum-/Datenschutz-Link, Abmelde-Text), pro Funnel-Mail abschaltbar (`funnel_mails.baustein_*`). Jede Funnel-/System-Mail läuft über `baueMailHtml()`. Abmeldelink = HMAC-Token über Empfänger-Art+ID (`/abmelden/[token]` mit Bestätigungsklick, One-Click-POST unter `/api/public/abmelden/[token]` + `List-Unsubscribe`-Header); Abmeldung landet in `mail_abmeldungen` (per E-Mail, gilt auch für Leads/Warteliste) und setzt Teilnehmer auf `abgemeldet`. System-Mails (transaktional) bekommen nie einen Abmeldelink. `mail-html.ts` ist ohne Node-Abhängigkeiten, weil die Backstage-Vorschau im Browser es nutzt.
