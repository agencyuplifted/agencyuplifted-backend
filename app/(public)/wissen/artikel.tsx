...
// cache(): generateMetadata und die Seite brauchen denselben Eintrag -- so
// nur eine DB-Abfrage pro Rendering.
export const ladeVeroeffentlichtenEintrag = cache(async function ladeVeroeffentlichtenEintrag(slug: string) {
...
// Entwurfsvorschau (Route /wissen/vorschau/[id], verlinkt aus dem Backstage-
// Editor): per ID, egal welcher Status -- die UUID dient als unratbares Token,
// kein zusaetzliches Secret noetig. Die oeffentliche Artikelseite zeigt
// dagegen nur Veroeffentlichtes.
export async function ladeEintragPerId(id: string) {
  const supabase = getSupabaseAdmin();
  const { data } = await supabase.from("insights_eintraege").select("*").eq("id", id).maybeSingle();
  return data;
}
...
// Kanonische Basis-URL aus PUBLIC_HOST statt aus dem Request-Host: headers()
// wuerde jede Artikelseite dynamisch machen und das Zwischenspeichern (ISR)
// verhindern. Auf der Backstage-Domain sind die Seiten ohnehin noindex.
async function basisUrl() {
  const host = (process.env.PUBLIC_HOST || "").split(",")[0]?.trim() || "backstage.agencyuplifted.com";
  return `https://${host}`;
}
...
export async function artikelMetadata(eintrag: any): Promise<Metadata> {
...
export async function ArtikelSeite({ eintrag }: { eintrag: any }) {
  const istVorschau = eintrag.status !== "veroeffentlicht";
